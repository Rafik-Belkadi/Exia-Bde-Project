					...
					<div class="container">
						...
						<div class="box alt">
							<div class="row uniform">
								...
								$wtf = $_POST['pdf'];
								$product_req = $bdd->prepare('SELECT * FROM products WHERE name="'.$wtf.'" ');
                                $product_req->execute();
                                    
                                $requete = $bdd->prepare('SELECT count(*) as total FROM products WHERE name="'.$wtf.'"');

                                $requete->execute();
                                $nombre_rows = $requete->fetch(PDO::FETCH_OBJ);
                                $nombre = $nombre_rows->total;
                                if($nombre > 0 ){
                                    ...
                                    echo '<h2>Aucun produit ne corresponds</h2>';
                                }
								?>
							</div>
						</div> 
